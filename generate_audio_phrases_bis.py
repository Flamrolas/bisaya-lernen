#!/usr/bin/env python3
"""
Generiert die fehlenden Bisaya-Audiodateien fuer die Wendungen (PHRASES_VOCAB) per Azure Speech.
...
Vorbereitung:
  ...
  3. Starten:
     python3 generate_audio_phrases_bis.py

Ergebnis: neue MP3s im Ordner "audio/" + Datei "phrases_audio_map_bis.json".
...
AUDIO_DIR = "audio"
MANIFEST_FILE = "phrases_audio_map_bis.json"
TEXTS_FILE = "phrases_missing_bis.json"
...
def synth_one(text):
    ...
    body = ssml_for(text, VOICE, "fil-PH").encode("utf-8")
    last_err = "unbekannt"
...
def main():
    ...
    print(f"Starte Generierung fuer {len(texts)} Bisaya-Wendungen (Stimme: {VOICE}) ...")
    ok, skip, err = 0, 0, 0
    errors = []
